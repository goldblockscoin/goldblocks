...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("goldblocks"))
        return false;
...
{
    // Convert goldblocks:// to goldblocks:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("goldblocks://"))
    {
        uri.replace(0, 12, "goldblocks:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GoldBlocks.lnk";
}
...
{
    return GetAutostartDir() / "goldblocks.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("GoldBlocks-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  goldblocks-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("GoldBlocks-Qt"));
